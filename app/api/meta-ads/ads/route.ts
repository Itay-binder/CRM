...
import { listAds } from "@/lib/metaAds/graph";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const ads = await listAds(config, datePreset);
    return NextResponse.json({ ok: true, datePreset, fetchedAt: new Date().toISOString(), ads });
  } catch (e) {
    return NextResponse.json(
